...
the model\nbsp{}--- /control flow objects/ (or /kernels/) and
...
\end{align*}
Here \(\eps\) is white noise and \(C_t\) includes \(dk\). Substituting integral
with infinite sum yields two-dimensional form of eq.\nbsp{}[[eq:longuet-higgins]].
