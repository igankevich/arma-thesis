...
in popular YARN job scheduler cite:vavilapalli2013yarn, which is used by Hadoop
...
standby reserved server cite:murthy2011architecture. Both principal servers are
coordinated by Zookeeper service which itself uses dynamic role assignment to
ensure its fault-tolerance cite:okorafor2012zookeeper. So, the whole setup is
